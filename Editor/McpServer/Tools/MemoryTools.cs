    ...
    public partial class McpUnityServer
    {
        ...
        private static Dictionary<string, object> LoadMemoryCache()
        {
            ...
            try
            {
                ...
                // Use ParseJsonObject (supports Dictionary) instead of JsonUtility.FromJson (which doesn't)
                return JsonHelper.ParseJsonObject(json) ?? CreateEmptyCache();
            }
            catch (Exception e)
            {
        ...
        {
            var section = ArgumentParser.GetString(args, "section", "all").ToLower();
            ...
            if (section == "all")
            {
                cache.TryGetValue("version", out var version);
                cache.TryGetValue("lastUpdated", out var lastUpdated);
                cache.TryGetValue("projectName", out var projectName);
                cache.TryGetValue("assets", out var assets);
                cache.TryGetValue("scenes", out var scenes);
                cache.TryGetValue("hierarchy", out var hierarchy);
                cache.TryGetValue("operations", out var operations);
                ...
                {
                    ["assets"] = assets == null || IsSectionStale(cache, "assets"),
                    ["scenes"] = scenes == null || IsSectionStale(cache, "scenes"),
                    ["hierarchy"] = hierarchy == null || IsSectionStale(cache, "hierarchy")
                };
            }
            ...
            {
                return McpToolResult.Error($"Unknown section: {section}. Valid: assets, scenes, hierarchy, operations, all");
            }
        ...
        {
            var (section, sectionErr) = RequireArg(args, "section");
            ...
            {
                return McpToolResult.Error($"Unknown section: {section}. Valid: assets, scenes, hierarchy, all");
            }

            SaveMemoryCache(cache);
            // LogOperation removed — writing to disk on every refresh caused excessive I/O
