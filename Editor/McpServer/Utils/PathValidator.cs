...
namespace McpUnity.Utils
{
    ...
    public static class PathValidator
    {
        ...
        public static string SanitizePath(string path, string requiredPrefix = "Assets/")
        {
            ...
            // Convert relative Assets/ path to absolute, resolve, then verify it's still in project
            string projectRoot = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string resolvedPath = Path.GetFullPath(path);
            if (!resolvedPath.StartsWith(projectRoot, StringComparison.OrdinalIgnoreCase))
                throw new ArgumentException("Resolved path points outside the project (possible symlink traversal)");
