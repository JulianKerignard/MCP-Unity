...
namespace McpUnity.Server
{
    ...
    public partial class McpUnityServer
    {
        ...
        /// <summary>
        /// Tracks whether the server was running before a domain reload.
        /// SessionState persists across recompilations but not across Editor sessions.
        /// </summary>
        private static bool WasRunningBeforeReload
        {
            get => SessionState.GetBool("McpUnity_WasRunning", false);
            set => SessionState.SetBool("McpUnity_WasRunning", value);
        }
        ...
        private static void ProcessMessageQueue()
        {
            ...
            if (!_initialized && !EditorApplication.isCompiling)
            {
                ...
                if (_startupFrameCount > McpConstants.StartupFrameDelay)
                {
                    ...
                    if ((AutoStart || WasRunningBeforeReload) && !_isRunning)
                    {
                        McpDebug.Log("[MCP Unity] Auto-restart after domain reload");
                        Start();
                    }
                }
            }
        ...
        private static void OnEditorLoaded()
        {
            ...
            if (AutoStart || WasRunningBeforeReload)
            {
                Start();
            }
        }
        ...
        private static void InitializeRegistries()
        {
            ...
            // Restore previously enabled categories from EditorPrefs
            _toolRegistry.RestoreEnabledCategories();
        ...
        /// </summary>
        [MenuItem("Conductor Windows/Start Server", priority = 1)]
        ...
        [MenuItem("Conductor Windows/Stop Server", priority = 2)]
