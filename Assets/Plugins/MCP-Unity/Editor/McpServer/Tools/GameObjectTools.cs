...
namespace McpUnity.Server
{
    ...
    public partial class McpUnityServer
    {
        ...
        {
            if (!args.TryGetValue("objects", out var objectsObj))
                return McpToolResult.Error("Missing required parameter: 'objects'");

            List<object> objectsRaw;
            if (objectsObj is List<object> list)
            {
                objectsRaw = list;
            }
            else if (objectsObj is object[] arr)
            {
                objectsRaw = new List<object>(arr);
            }
            else
            {
                return McpToolResult.Error($"'objects' must be an array, got: {objectsObj?.GetType().Name ?? "null"}");
            }

            if (objectsRaw.Count == 0)
                return McpToolResult.Error("'objects' array must not be empty.");
