...
namespace McpUnity.Server
{
    ...
    public partial class McpUnityServer
    {
        ...
        private static readonly HashSet<string> AllowedMenuPaths = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            "Tools/TD Diagnose References",
            "Tools/TD Configure LayerMasks",
            "Tools/TD Stop and Save",
            "Tools/TD Add New Enemies to Waves",
            "Tools/TD Set Blocked Layer",
        ...
        static partial void RegisterEditorWorkflowTools()
        {
            _toolRegistry.RegisterTool(new McpToolDefinition
            {
                ...
                inputSchema = new McpInputSchema
                {
                    ...
                    properties = new Dictionary<string, McpPropertySchema>
                    {
                        ...
                        ["allowCustom"] = new McpPropertySchema
                        {
                            type = "boolean",
                            description = "Allow executing project-specific menu items under Tools/ (default: false)"
                        }
                    },
            ...
            _toolRegistry.RegisterTool(new McpToolDefinition
            {
                ...
                description = "Control Unity Play mode: start, stop, or pause the game",
                inputSchema = new McpInputSchema
                {
                    type = "object",
                    properties = new Dictionary<string, McpPropertySchema>
                    {
                        ...
                        {
                            type = "string",
                            description = "Action to perform: 'play' (start/resume), 'stop' (exit play mode), 'pause' (toggle pause)",
                            @enum = new List<string> { "play", "stop", "pause" }
                        }
                    },
        ...
        private static McpToolResult ExecuteMenuItem(Dictionary<string, object> args)
        {
            ...
            bool allowCustom = ArgumentParser.GetBool(args, "allowCustom", false);

            if (!AllowedMenuPaths.Contains(menuPath))
            {
                // Allow Tools/ prefix when allowCustom is true
                if (allowCustom && menuPath.StartsWith("Tools/", StringComparison.OrdinalIgnoreCase))
                {
                    McpDebug.Log($"[MCP Unity] Executing custom menu item: {menuPath}");
                }
                else
                {
                    McpDebug.LogWarning($"[MCP Unity] Blocked menu item execution (not in allowlist): {menuPath}");
                    return McpToolResult.Error($"Menu item not allowed for security reasons: {menuPath}. Use allowed menu paths only, or set allowCustom=true for Tools/ items.");
                }
            }
        ...
        private static McpToolResult SetPlayMode(Dictionary<string, object> args)
        {
                ...
                switch (action.ToLowerInvariant())
                {
                    case "play":
                        if (EditorApplication.isPlaying)
                        {
                            if (EditorApplication.isPaused)
                            {
                                ...
                                return McpResponse.Success("Resumed play mode", new { isPlaying = true, isPaused = false });
                            }
                            return McpResponse.Success("Already in play mode", new { isPlaying = true, isPaused = false });
                        }
                        EditorApplication.isPlaying = true;
                        return McpResponse.Success("Entering play mode", new { isPlaying = true, isPaused = false });

                    case "stop":
                        if (!EditorApplication.isPlaying)
                            return McpResponse.Success("Already in edit mode", new { isPlaying = false, isPaused = false });
                        EditorApplication.isPlaying = false;
                        return McpResponse.Success("Exiting play mode", new { isPlaying = false, isPaused = false });
                    ...
                    case "pause":
                        ...
                        return McpResponse.Success(
                            EditorApplication.isPaused ? "Paused" : "Unpaused",
                            new { isPlaying = true, isPaused = EditorApplication.isPaused });

                    default:
                        return McpToolResult.Error($"Unknown action: {action}. Use 'play', 'stop', or 'pause'.");
                }
            }
            catch (Exception ex)
            {
                return McpToolResult.Error($"Failed to set play mode: {ex.Message}");
            }
        }
