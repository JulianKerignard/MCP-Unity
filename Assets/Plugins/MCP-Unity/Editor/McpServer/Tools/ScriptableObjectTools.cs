...
namespace McpUnity.Server
{
    ...
    public partial class McpUnityServer
    {
        ...
        private static object ConvertValue(object value, Type targetType)
        {
            ...
            // Handle Unity Object references (GameObject, ScriptableObject, Material, etc.)
            if (typeof(UnityEngine.Object).IsAssignableFrom(underlyingType) && value is string assetPath)
            {
                if (assetPath.StartsWith("Assets/", StringComparison.OrdinalIgnoreCase))
                {
                    var asset = AssetDatabase.LoadAssetAtPath(assetPath, underlyingType);
                    if (asset != null) return asset;
                    // Try loading as generic Object and let Unity handle type
                    asset = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(assetPath);
                    if (asset != null) return asset;
                }
                McpUnity.Editor.McpDebug.LogWarning($"[ScriptableObject] Asset not found at: {assetPath}");
                return null;
            }

            // Handle arrays
            if (underlyingType.IsArray && value is List<object> arrayItems)
            {
                var elementType = underlyingType.GetElementType();
                var array = Array.CreateInstance(elementType, arrayItems.Count);
                for (int i = 0; i < arrayItems.Count; i++)
                {
                    array.SetValue(ConvertValue(arrayItems[i], elementType), i);
                }
                return array;
            }

            // Handle List<T>
            if (underlyingType.IsGenericType && underlyingType.GetGenericTypeDefinition() == typeof(List<>) && value is List<object> listItems)
            {
            ...
            // Handle serializable classes/structs (dictionaries from JSON)
            if (value is Dictionary<string, object> objDict && !underlyingType.IsPrimitive && underlyingType != typeof(string))
            {
                // Skip types already handled above (Vector2, Vector3, Color)
                if (underlyingType != typeof(Vector2) && underlyingType != typeof(Vector3) && underlyingType != typeof(Color))
                {
                    var instance = Activator.CreateInstance(underlyingType);
                    foreach (var kvp in objDict)
                    {
                        var field = underlyingType.GetField(kvp.Key, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                        if (field != null)
                        {
                            field.SetValue(instance, ConvertValue(kvp.Value, field.FieldType));
                        }
                    }
                    return instance;
                }
            ...
            {
                McpUnity.Editor.McpDebug.LogWarning($"[ScriptableObject] Cannot convert '{value}' ({value.GetType().Name}) to {underlyingType.Name}: {ex.Message}");
                return null;
            }
        }
