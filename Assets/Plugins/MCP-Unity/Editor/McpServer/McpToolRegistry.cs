...
namespace McpUnity.Server
{
    ...
    public class McpToolRegistry
    {
        ...
        private const string EditorPrefsKey = "McpUnity_EnabledCategories";
        ...
        /// <summary>
        /// Restore enabled categories from EditorPrefs.
        /// Call this AFTER all tools have been registered.
        /// </summary>
        public void RestoreEnabledCategories()
        {
            string saved = EditorPrefs.GetString(EditorPrefsKey, "");
            if (string.IsNullOrEmpty(saved)) return;

            var categories = saved.Split(',');
            int restored = 0;
            foreach (var cat in categories)
            {
                var trimmed = cat.Trim();
                if (!string.IsNullOrEmpty(trimmed) && !_enabledCategories.Contains(trimmed))
                {
                    // Verify category actually exists in registered tools
                    bool exists = false;
                    foreach (var registeredCat in _toolCategories.Values)
                    {
                        if (string.Equals(registeredCat, trimmed, StringComparison.OrdinalIgnoreCase))
                        {
                            _enabledCategories.Add(registeredCat);
                            exists = true;
                            restored++;
                            break;
                        }
            ...
            {
                _cachedToolList = null;
                McpDebug.Log($"[MCP Registry] Restored {restored} categories from EditorPrefs ({VisibleCount}/{Count} tools visible)");
            }
        }

        private void SaveEnabledCategories()
        {
            EditorPrefs.SetString(EditorPrefsKey, string.Join(",", _enabledCategories));
        }
