...
namespace McpUnity.Editor
{
    ...
    public class McpServerLogger
    {
        ...
        /// <summary>
        /// Thread-safe logging that stores the entry without firing OnLogAdded.
        /// Use from background threads (WebSocket, async operations) to avoid
        /// cross-thread calls to Unity APIs like EditorWindow.Repaint().
        /// The Diagnostics tab will pick up these logs on its next periodic repaint.
        /// </summary>
        public void LogThreadSafe(LogLevel level, string message, string context = null)
        {
            StoreEntry(level, message, context, fireEvent: false);
        }

        private void AddLog(LogLevel level, string message, string context)
        {
            StoreEntry(level, message, context, fireEvent: true);
        }

        private void StoreEntry(LogLevel level, string message, string context, bool fireEvent)
        {
            ...
            // Also log to Unity console (if enabled) — Debug.Log is thread-safe
            ...
            // Notify listeners — only from main thread to avoid cross-thread UI calls
            if (fireEvent)
            {
